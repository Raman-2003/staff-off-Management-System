...
const OUTPUT_FILE = 'linkedin_results.csv';
...
const curlCommand = `curl -s -X GET \\
  "https://www.linkedin.com/jobapi/v3/search?noOfResults=20&urlType=search_by_key_loc&searchType=adv&keyword=${encodeURIComponent(SEARCH_KEYWORD)}&experience=${EXPERIENCE_RANGE}&k=${encodeURIComponent(SEARCH_KEYWORD)}&seoKey=node-js-jobs&src=jobsearchDesk&latLong=" \\
  -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" \\
  -H "Accept: application/json, text/plain, */*" \\
  -H "Accept-Language: en-US,en;q=0.9" \\
  -H "Referer: https://www.linkedin.com/node-js-jobs?experience=${EXPERIENCE_RANGE}" \\
...
try {
  ...
  if (data && data.jobDetails && Array.isArray(data.jobDetails)) {
    ...
    return data.jobDetails.map(job => ({
      ...
      url: `https://www.linkedin.com/job-listings-${job.jobId}`
    }));
  } else {
...
console.log('Starting enhanced linkedin crawler...');
...
page.on('request', (request) => {
  ...
  if (request.resourceType() === 'document' || request.resourceType() === 'xhr') {
    ...
    headers['Referer'] = 'https://www.google.com/search?q=node.js+jobs+linkedin';
...
await randomDelay(5000); // wait after login
console.log('✅ Logged in to LinkedIn (assuming no CAPTCHA)');

// Scroll for 5 seconds before navigation
await page.evaluate(async () => {
...
// Instead of direct navigation to search results URL,
// manually type the search keyword into the search bar
console.log("on feed");
console.log("serching place");
// Wait for the search input to appear
await page.waitForSelector('input[placeholder="Search"]', { visible: true });
console.log("found place and clicking on it");
// Click on the search input
await page.click('input[placeholder="Search"]');

// Type your search keyword
await page.keyboard.type(SEARCH_KEYWORD, { delay: 100 });
...
console.log("✅ Done typing and searching on LinkedIn");
